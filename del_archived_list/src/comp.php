...
// Deletes supplied archived lists
...
// Create report file
$logfile = "../../report_files/del_archived_list_output.log";
$myfile = fopen("../../report_files/del_archived_list_output.log", "a") or die("Unable to open del_archived_list_output.log");
...
while (!feof($file_handle)) {
	...
	// Get list data
	$list_data = get_list($shortCode, $listID, $token);
	if (!$list_data) {
		fwrite($myfile, "List data not retrieved. Skipping... \r\n");
		continue;
	}

	// Check published_status
	if (isset($list_data->data->attributes->published_status)) {
		$pubstatus = $list_data->data->attributes->published_status;
	} else {
		$pubstatus = "";
	}
	
	if ($pubstatus !== "archived") {
		echo_message_to_screen(WARNING, "List ID: $listID the wrong published status - it is $pubstatus. This tool only deletes archives lists. \r\n");
		fwrite($myfile, "Wrong published status ($pubstatus). \r\n");
		continue;
	}
...
// Get list data
function get_list($shortCode, $listID, $token) {
